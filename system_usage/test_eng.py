...
names = [
    ("CPU (%)", "CPU with VBO", "CPU without VBO", "tab:red", "tab:pink", df_vbo["cpu_percent"], df_no_vbo["cpu_percent"]),
    ("RAM (MB)", "RAM with VBO", "RAM without VBO", "tab:blue", "tab:cyan", df_vbo["ram_used_mb"], df_no_vbo["ram_used_mb"]),
    ("GPU (%)", "GPU with VBO", "GPU without VBO", "tab:green", "lightgreen", df_vbo["gpu_percent"], df_no_vbo["gpu_percent"]),
    ("VRAM (MB)", "VRAM with VBO", "VRAM without VBO", "tab:purple", "violet", df_vbo["vram_used_mb"], df_no_vbo["vram_used_mb"])
]
...
for ylabel, label1, label2, color1, color2, data1, data2 in names:
    ...
    plt.xlabel("Step (s)", fontsize=label_fontsize, fontname='Times New Roman', fontweight='bold')
