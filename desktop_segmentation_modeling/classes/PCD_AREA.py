...
class PCD_AREA(PCD):
    ...
    def auto_set_z_bounds(self, cs, percentile_low=5, percentile_up=95):
        """ Определяет LOW и UP на основе процентилей данных """
        if self.points is None or self.points.shape[0] == 0:
            print("⚠️ Ошибка: Невозможно определить LOW и UP, так как облако пустое!")
            return

        cs.LOW = np.percentile(self.points[:, 2], percentile_low)
        cs.UP = np.percentile(self.points[:, 2], percentile_up)
        print(f"Автоматически определённый диапазон Z: {cs.LOW} - {cs.UP}")
