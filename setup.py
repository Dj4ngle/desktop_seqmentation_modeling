# -*- coding: utf-8 -*-

from setuptools import setup, find_packages

# --- Метаданные ---
PACKAGE_NAME = 'desktop_segmentation_modeling'
VERSION = '1.0.6'  # Начальная версия, которую вы можете обновить
DESCRIPTION = 'A powerful desktop application for 3D point cloud and image segmentation using machine learning.'
URL = 'https://github.com/Dj4ngle/desktop_seqmentation_modeling'
AUTHOR = 'Dj4ngle'
AUTHOR_EMAIL = ''  # Укажите ваш email, если хотите
PYTHON_VERSION = '>=3.11'  # Целевая версия Python

# --- Список Зависимостей ---
# Включает все перечисленные библиотеки, необходимые для работы проекта
INSTALL_REQUIRES = [
    'altgraph==0.17.4',
    'ansi2html==1.9.1',
    'asciitree==0.3.3',
    'asttokens==2.4.1',
    'attrs==23.2.0',
    'blinker==1.7.0',
    'certifi==2024.2.2',
    'charset-normalizer==3.3.2',
    'circle-fit==0.2.1',
    'click==8.1.7',
    'colorama==0.4.6',
    'comm==0.2.1',
    'ConfigArgParse==1.7',
    'contourpy==1.2.1',
    'cycler==0.12.1',
    'dash==2.14.2',
    'dash-core-components==2.0.0',
    'dash-html-components==2.0.0',
    'dash-table==5.0.0',
    'decorator==5.1.1',
    'Deprecated==1.2.18',
    'docopt==0.6.2',
    'exceptiongroup==1.2.0',
    'executing==2.0.1',
    'fasteners==0.19',
    'fastjsonschema==2.19.1',
    'filelock==3.17.0',
    'Flask==3.0.1',
    'fonttools==4.51.0',
    'fsspec==2025.2.0',
    'gitdb==4.0.12',
    'GitPython==3.1.44',
    'hdbscan==0.8.40',
    'idna==3.7',
    'imageio==2.37.0',
    'importlib-metadata==7.0.1',
    'importlib_resources==6.4.0',
    'ipython==8.18.1',
    'ipywidgets==8.1.1',
    'itsdangerous==2.1.2',
    'jedi==0.19.1',
    'Jinja2==3.1.3',
    'joblib==1.4.2',
    'jsonschema==4.21.1',
    'jsonschema-specifications==2023.12.1',
    'jupyter_core==5.7.1',
    'jupyterlab-widgets==3.0.9',
    'kiwisolver==1.4.5',
    'laspy==2.5.4',
    'lazrs==0.6.2',
    'lazy_loader==0.4',
    'llvmlite==0.43.0',
    'lxml==5.3.1',
    'MarkupSafe==2.1.5',
    'matplotlib==3.9.0',
    'matplotlib-inline==0.1.6',
    'mpmath==1.3.0',
    'nbformat==5.9.2',
    'nest-asyncio==1.6.0',
    'networkx==3.2.1',
    'numba==0.60.0',
    'numcodecs==0.12.1',
    'numpy==1.26.4',
    'open3d==0.18.0',
    'opencv-python==4.7.0.68',
    'packaging==24.0',
    'pandas==2.2.2',
    'parso==0.8.3',
    'pefile==2023.2.7',
    'phasepack==1.5',
    'pillow==10.3.0',
    'platformdirs==4.1.0',
    'plotly==5.18.0',
    'pooch==1.8.1',
    'prompt-toolkit==3.0.43',
    'pure-eval==0.2.2',
    'py-cpuinfo==9.0.0',
    'Pygments==2.17.2',
    'pyinstaller==6.7.0',
    'pyinstaller-hooks-contrib==2024.6',
    'pylas==0.4.3',
    'pyntcloud==0.3.1',
    'PyOpenGL==3.1.7',
    'pyparsing==3.1.2',
    'pypcd',
    'PyQt6==6.6.1',
    'PyQt6-Qt6==6.6.1',
    'PyQt6-sip==13.6.0',
    'PyReconstruct==1.9.3',
    'pyshp==2.3.1',
    'PySide6==6.5.2',
    'PySide6-Addons==6.5.2',
    'PySide6-Essentials==6.5.2',
    'python-dateutil==2.9.0.post0',
    'python-lzf==0.2.6',
    'pytz==2024.1',
    'pyvista==0.43.8',
    'PyWavefront==1.3.3',
    'pywin32==306',
    'pywin32-ctypes==0.2.2',
    'QDarkStyle==3.2.3',
    'QtPy==2.4.3',
    'referencing==0.32.1',
    'requests==2.32.2',
    'retrying==1.3.4',
    'rpds-py==0.17.1',
    'ruamel.yaml.clib==0.2.12',
    'scikit-image==0.24.0',
    'scikit-learn==1.5.0',
    'scipy==1.13.1',
    'scooby==0.10.0',
    'seaborn==0.13.2',
    'shapely==2.0.7',
    'shiboken6==6.5.2',
    'SimpleITK==2.4.1',
    'six==1.16.0',
    'smmap==5.0.2',
    'stack-data==0.6.3',
    'sympy==1.13.1',
    'tenacity==8.2.3',
    'threadpoolctl==3.5.0',
    'tifffile==2024.8.30',
    'torch==2.6.0',
    'tqdm==4.67.1',
    'traitlets==5.14.1',
    'trimesh==3.18.1',
    'typing_extensions==4.12.2',
    'tzdata==2024.1',
    'urllib3==2.2.1',
    'vedo==2023.4.7',
    'vtk==9.3.0',
    'wcwidth==0.2.13',
    'Werkzeug==3.0.1',
    'widgetsnbextension==4.0.9',
    'wrapt==1.17.2',
    'zarr==2.18.2',
    'zipp==3.17.0',
]

DEPENDENCY_LINKS = [
    'git+https://github.com/DanielPollithy/pypcd.git#egg=pypcd'
]

# --- Исполнение setup ---
setup(
    name=PACKAGE_NAME,
    version=VERSION,
    description=DESCRIPTION,
    long_description=open('README.md', encoding='utf-8').read(),
    long_description_content_type='text/markdown',
    url=URL,
    author=AUTHOR,
    author_email=AUTHOR_EMAIL,
    license='MIT',  # Часто используемая открытая лицензия, рекомендуется добавить LICENSE файл
    packages=find_packages(),
    install_requires=INSTALL_REQUIRES,
    dependency_links=DEPENDENCY_LINKS,

    package_data={
        'desktop_segmentation_modeling': ['style.qss', 'images/*'],
    },
    include_package_data=True,

    # Классификаторы помогают людям найти ваш пакет на PyPI
    classifiers=[
        'Development Status :: 4 - Beta',  # Укажите актуальный статус
        'Intended Audience :: Science/Research',
        'Intended Audience :: Developers',
        'License :: OSI Approved :: MIT License',
        'Programming Language :: Python :: 3',
        'Programming Language :: Python :: 3.11',
        'Topic :: Scientific/Engineering :: GIS',
        'Topic :: Scientific/Engineering :: Image Processing',
        'Operating System :: OS Independent',
    ],
    python_requires=PYTHON_VERSION,
)